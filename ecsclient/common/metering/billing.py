...
class Billing(object):
    ...
    def get_bucket_billing_info(self, bucket_name, namespace, sizeunit="GB"):
        ...
        return self.conn.get(
            url="object/billing/buckets/{0}/{1}/info".format(namespace, bucket_name),
            params=params,
        )

    def get_namespace_billing_info(
        self, namespace, sizeunit="GB", include_bucket_detail=False, marker=None
    ):
        """
        ...
        params = {"include_bucket_detail": include_bucket_detail, "sizeunit": sizeunit}

        if marker:
            params["marker"] = marker

        return self.conn.get(
            url="object/billing/namespace/{0}/info".format(namespace), params=params
        )
    ...
    def get_namespace_billing_sample(
        ...
        return self.conn.get(
            url="object/billing/namespace/{0}/sample".format(namespace), params=params
        )

    def get_bucket_billing_sample(
        self, bucket_name, namespace, start_time, end_time, sizeunit="GB"
    ):
        """
        Gets billing details for the specified namespace, interval and bucket
        ...
        """
        log.info(
            "Sampling billing info for bucket '{0}' in namespace "
            "'{1}'".format(bucket_name, namespace)
        )

        params = {"start_time": start_time, "end_time": end_time, "sizeunit": sizeunit}

        return self.conn.get(
            url="object/billing/buckets/{0}/{1}/sample".format(namespace, bucket_name),
            params=params,
        )
