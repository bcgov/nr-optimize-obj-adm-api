...
class Bucket(object):
    ...
    def list(self, namespace, marker="", limit=100):
        ...
        return self.conn.get(
            "object/bucket?namespace={namespace}&marker={marker}&limit={limit}".format(
                namespace=namespace, marker=marker, limit=limit
            )
        )

    def getbucketdetails(self, namespace, bucketname):
        """
        Gets the size details for a single bucket for the specified namespace.
...

        {
            'namespace': 'nrs',
            'name': 'bzbolu',
            'vpool_id': 'urn:storageos:ReplicationGroupInfo:4759217e-b060-4abb-a100-6a50686d6cf8:global',
            'total_size': '1038.4766',
            'total_size_unit': 'GB',
            'total_objects': 93897,
            'sample_time': '2021-05-27T00:08:39Z',
            'uptodate_till': '2021-05-17T08:09:08Z',
            'total_mpu_size': '0',
            'total_mpu_parts': 0,
            'TagSet': [{
                    'Key': 'Custodian',
                    'Value': 'first.last@gov.bc.ca'
                }, {
                    'Key': 'Organization',
                    'Value': 'IITD GIS Services'
                }, {
                    'Key': 'Environment',
                    'Value': 'Test / Archive'
                }
            ],
            'total_objects_deleted': '20277',
            'total_size_deleted': '456.0248'
        }
        ...
        return self.conn.get(
            "object/billing/buckets/{namespace}/{bucketname}/info".format(
                namespace=namespace, bucketname=bucketname
            )
        )
    ...
    def set_retention(self, bucket_name, namespace, period=2592000):
        ...
        """
        payload = {"period": period, "namespace": namespace}

        log.info(
            "Setting retention for bucket '{0}' in namespace "
            "'{1}': {2}".format(bucket_name, namespace, payload)
        )

        return self.conn.put(
            "object/bucket/{}/retention".format(bucket_name), json_payload=payload
        )
    ...
    def set_owner(self, bucket_name, new_owner, namespace=None):
        ...
        return self.conn.post(
            "object/bucket/{}/owner".format(bucket_name), json_payload=payload
        )
    ...
    def set_stale_allowed(self, bucket_name, stale_allowed, namespace=None):
        ...
        log.info(
            "Updating 'isStaleAllowed' for bucket '{}': {}".format(
                bucket_name, stale_allowed
            )
        )

        return self.conn.post(
            "object/bucket/{}/isstaleallowed".format(bucket_name), json_payload=payload
        )
    ...
    def set_lock(self, bucket_name, is_locked=False, namespace=None):
        ...
        log.info("Setting lock to '{}' for bucket '{}'".format(is_locked, bucket_name))
    ...
    def set_quota(self, bucket_name, block_size, notification_size, namespace=None):
        ...
        """
        payload = {"blockSize": block_size, "notificationSize": notification_size}
        ...
        return self.conn.put(
            "object/bucket/{0}/quota".format(bucket_name), json_payload=payload
        )
    ...
    def delete_quota(self, bucket_name, namespace=None):
        ...
        return self.conn.delete(
            url="object/bucket/{}/quota".format(bucket_name), params=params
        )
    ...
    def get_acl(self, bucket_name, namespace=None):
        ...
        return self.conn.get("object/bucket/{}/acl".format(bucket_name), params=params)
    ...
    def set_acl(
        ...
        """
        payload = {"bucket": bucket_name, "acl": {}}
        ...
        self.conn.put("object/bucket/{}/acl".format(bucket_name), json_payload=payload)
    ...
    def get_metadata(self, bucket_name, head_type, namespace=None):
        ...
        return self.conn.get(
            "object/bucket/{}/metadata".format(bucket_name), params=params
        )

    def set_metadata(
        self, bucket_name, metadata_key, metadata_value, head_type, namespace=None
    ):
        """
        ...
        payload = {
            "head_type": head_type,
            "metadata": [{"name": metadata_key, "value": metadata_value}],
        }
    ...
    def delete_metadata(self, bucket_name, head_type, namespace=None):
        ...
        log.info(
            "Deleting metadata for bucket '{}', head '{}'".format(
                bucket_name, head_type
            )
        )
        return self.conn.delete(
            "object/bucket/{}/metadata".format(bucket_name), params=params
    ...
    def disable_search_metadata(self, bucket_name, namespace=None):
        ...
        log.info(
            "Disabling search metadata functionality for bucket '{}'".format(
                bucket_name
            )
        )

        return self.conn.delete(
            "object/bucket/{}/searchmetadata".format(bucket_name), params=params
        )
