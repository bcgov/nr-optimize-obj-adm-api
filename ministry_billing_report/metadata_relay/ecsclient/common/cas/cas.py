...
class Cas(object):
    ...
    def get_cas_secret(self, uid, namespace=None):
        ...
        """
        log.info(
            "Fetching CAS secret for user ID '{uid}' (namespace '{namespace}')"
        ).format(uid=uid, namespace=namespace)
        if namespace:
    ...
    def set_cas_secret(self, uid, namespace, secret):
        ...
        """
        payload = {"user_cas_secret_param": {"namespace": namespace, "secret": secret}}

        log.info("Updating CAS secret for user ID '{}'".format(uid))
        return self.conn.post(
            "object/user-cas/secret/{}".format(uid), json_payload=payload
        )
    ...
    def delete_cas_secret(self, uid, namespace, secret):
        ...
        """
        payload = {"namespace": namespace, "secret": secret}

        log.info("Deleting CAS secret for user ID '{}'".format(uid))
        return self.conn.post(
            "object/user-cas/secret/{}/deactivate".format(uid), json_payload=payload
        )
    ...
    def generate_pea(self, uid, namespace):
        ...
        """
        log.info(
            "Generating Pool Entry Authorization (PEA) file for user ID '{uid}' (namespace '{namespace}')"
        ).format(uid=uid, namespace=namespace)
    ...
    def get_default_bucket(self, uid, namespace=None):
        ...
        """
        log.info(
            "Getting default bucket for user ID '{uid}' (namespace '{namespace}')"
        ).format(uid=uid, namespace=namespace)
        if namespace:
    ...
    def set_default_bucket(self, uid, namespace, bucket_name):
        ...
        """
        log.info(
            "Setting default bucket to '{bucket_name}' for user ID '{uid}' (namespace '{namespace}')"
        ).format(uid=uid, namespace=namespace, bucket_name=bucket_name)
        payload = {"name": bucket_name}
        return self.conn.post(
            "object/user-cas/bucket/{namespace}/{uid}".format(
                uid=uid, namespace=namespace
            ),
    ...
    def get_registered_applications(self, namespace):
        ...
        log.info(
            "Getting registered CAS applications on namespace '{namespace}'"
        ).format(namespace=namespace)
        return self.conn.get(
            "object/user-cas/applications/{namespace}".format(namespace=namespace)
        )
    ...
    def get_user_metadata(self, uid, namespace):
        ...
        log.info(
            "Getting CAS user metadata for user ID '{uid}' (namespace '{namespace}')"
        ).format(uid=uid, namespace=namespace)
        return self.conn.get(
            "object/user-cas/metadata/{namespace}/{uid}".format(
                uid=uid, namespace=namespace
            )
        )
