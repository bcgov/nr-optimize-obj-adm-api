...
def main(argv):
    ...
    listDataFrame = pd.DataFrame(
        list(zip(listOfMainFolder, listOfMainFolderSize, listofMainCost))
    )
    listDataFrame.columns = ["Main Folder", "Folder Size GB", "Cost of Storage"]
    ...
    listDataFrame["Cost of Storage"] = listDataFrame["Cost of Storage"].apply(
        lambda x: f"${x:.2f}"
    )

    listDataFrame1 = pd.DataFrame(
        list(zip(listofFullPath, listofFullPathSize, listofSubCost))
    )
    listDataFrame1.columns = ["Subfolder", "Folder Size MB", "Cost of Storage"]
    ...
    listDataFrame1["Cost of Storage"] = listDataFrame1["Cost of Storage"].apply(
        lambda x: f"${x:.2f}"
    )

    # export to Excel, label the sheets
    with ExcelWriter(
        (outputdirectory) + r"\\Bucket_Folder_Sizes_" + (timestamp) + ".xlsx"
    ) as writer:
