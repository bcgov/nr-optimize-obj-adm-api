...
from boto3_helpers.checks import object_exists
from boto3_helpers.searches import get_uppercase_filenames
from utilities.file_helper import write_list_to_file
...
def main():
    ...
    upper_file_names = get_uppercase_filenames(s3_client)

    # Write the file names to a file
    bucket_name = constants.BUCKET_NAME
    write_list_to_file(f"{bucket_name}_extract_uppercase.txt", upper_file_names)
    ...
    for upper_name in upper_file_names:
        ...
        if object_exists(s3_client, upper_name.lower()):
    ...
    for upper_name in uppercase_folder_files:
        if upper_name.lower() in conflicted_folder_files:
            ...
            write_list_to_file(f"{bucket_name}_file_conflicts.txt", upper_file_names)
            ...
            write_list_to_file(f"{bucket_name}_process_after_conflicts.txt", upper_file_names)
        else:
            make_file_lowercase(s3_client, upper_name)
    # write_list_to_file(f"{bucket_name}_file_conflicts.txt", upper_file_names)
    # write_list_to_file(f"{bucket_name}_process_after_conflicts.txt", upper_file_names)
