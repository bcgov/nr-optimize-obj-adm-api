...
BUCKET_PATH_PREFIX = (os.getenv("BUCKET_PATH_PREFIX") or "").strip()

if "&amp;" in BUCKET_PATH_PREFIX:
    raise ValueError(
        "BUCKET_PATH_PREFIX contains '&amp;'. "
        "S3 keys use literal '&'. Fix the .env value."
    )

if not all([ACCESS_KEY, SECRET_KEY, S3_ENDPOINT, BUCKET_NAME]):
...
if BUCKET_PATH_PREFIX:
    if not BUCKET_PATH_PREFIX.endswith("/"):
        BUCKET_PATH_PREFIX = BUCKET_PATH_PREFIX + "/"
    paginate_kwargs["Prefix"] = BUCKET_PATH_PREFIX
    print(f"Running with prefix scope: {BUCKET_PATH_PREFIX}")
else:
...
scoped = f" (scoped to prefix: {BUCKET_PATH_PREFIX})" if BUCKET_PATH_PREFIX else ""
