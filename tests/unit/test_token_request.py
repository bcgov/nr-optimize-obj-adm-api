...
class TestTokenRequest(testtools.TestCase):
    ...
    def test_should_get_existing_token(self):
        with patch("os.path.isfile", return_value=True), patch(
            "six.moves.builtins.open", mock_open(read_data="123TOKEN"), create=True
        ):
            self.assertEqual(
                self.token_request._get_existing_token(), self.token_file_contents
            )
    ...
    def test_get_new_token_should_throw_ecsclientexception_401(self):
        self.requests_mock.register_uri(
            "GET", "https://127.0.0.1:4443/login", status_code=http_client.UNAUTHORIZED
        )
    ...
    def test_token_validation_401(self, mock_get_existing_token, mock_get_new_token):
        self.requests_mock.register_uri(
            "GET",
            "https://127.0.0.1:4443/user/whoami",
