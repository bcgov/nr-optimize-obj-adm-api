...
class TestBucket(functional.BaseTestCase):
    def __init__(self, *args, **kwargs):
        ...
        self.replication_group_1 = "functional-tests-replicationgroup-%s" % int(
            time.time()
        )
    ...
    def setUp(self):
        ...
        if len(r["data_service_vpool"]) > 0:
            ...
            zone_mappings = [(self.vdc_1_id, self.storage_pool_1_id)]
            r = self.client.replication_group.create(
                self.replication_group_1, zone_mappings
            )
    ...
    def tearDown(self):
        ...
        for bucket in [self.bucket_1, self.bucket_2]:
            try:
    ...
    def test_bucket_create(self):
        response = self.client.bucket.create(
            self.bucket_2,
            namespace=self.namespace_1,
            replication_group=self.replication_group_1_id,
    ...
    def test_bucket_set_owner(self):
        self.client.bucket.set_owner(
            self.bucket_1, self.object_user, namespace=self.namespace_1
        )
    ...
    def test_bucket_set_stale_allowed(self):
        ...
        self.client.bucket.set_stale_allowed(
            self.bucket_1, True, namespace=self.namespace_1
        )
    ...
    def test_bucket_lock(self):
        self.client.bucket.set_lock(
            self.bucket_1, is_locked=True, namespace=self.namespace_1
        )
        response = self.client.bucket.get_lock(
            self.bucket_1, namespace=self.namespace_1
        )
        self.assertEqual(response["isLocked"], True)

        self.client.bucket.set_lock(
            self.bucket_1, is_locked=False, namespace=self.namespace_1
        )
        response = self.client.bucket.get_lock(
            self.bucket_1, namespace=self.namespace_1
        )
    ...
    def test_bucket_quota(self):
        ...
        response = self.client.bucket.get_quota(
            self.bucket_1, namespace=self.namespace_1
        )
    ...
    def test_bucket_acl(self):
        ...
        customgroup_acl = [
            {"permission": ["delete", "read", "write"], "customgroup": "cgroup1"}
        ]

        self.client.bucket.set_acl(
            self.bucket_1,
            namespace=self.namespace_1,
        ...
        acl = self.client.bucket.get_acl(self.bucket_1, namespace=self.namespace_1)
        ...
        self.client.bucket.set_acl(self.bucket_1, namespace=self.namespace_1)
        acl = self.client.bucket.get_acl(self.bucket_1, namespace=self.namespace_1)
    ...
    def test_bucket_user_metadata(self):
        self.client.bucket.set_metadata(
            self.bucket_1, "key1", "value1", "S3", namespace=self.namespace_1
        )

        response = self.client.bucket.get_metadata(
            self.bucket_1, "S3", namespace=self.namespace_1
        )
        ...
        self.client.bucket.delete_metadata(
            self.bucket_1, "S3", namespace=self.namespace_1
        )
        f = self.client.bucket.get_metadata
        self.assertRaises(
            ECSClientException, f, self.bucket_1, "S3", namespace=self.namespace_1
        )
