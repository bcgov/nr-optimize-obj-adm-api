...
class TestDataStore(functional.BaseTestCase):
    def setUp(self):
        ...
        r = self.client.data_store.create(
            name=self.data_store_2_name,
    ...
    def test_data_store_get_by_storage_pool(self):
        ...
        self.assertEqual(
            response["commodity_data_store"][0]["varray"], self.storage_pool_1_id
        )

    def test_data_store_create(self):
        response = self.client.data_store.create(
            name=self.data_store_2_name,
            description="Data Store for functional test",
            node_id=self.data_store_1_id,
            storage_pool_id=self.storage_pool_1_id,
    ...
    def test_data_store_get_task(self):
        response = self.client.data_store.get_task(
            data_store_id=self.data_store_1_id, op_id=self.task_1_id
        )
