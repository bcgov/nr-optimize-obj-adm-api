...
class TestEcsClient(unittest.TestCase):
    ...
    def test_client_without_token_endpoint(self, mock_isfile):
        ...
        self.assertEqual(
            "'token_endpoint' not provided and missing 'token'|'token_path'",
            str(exception),
        )
    ...
    def test_client_without_credentials(self):
        ...
        exception = error.exception.message
        self.assertEqual(
            "'token_endpoint' provided but missing ('username','password')",
            str(exception),
        )
    ...
    def test_client_init_with_token(self):
        c = Client(version="3", token="1234567890", ecs_endpoint="https://192.168.1.10")
